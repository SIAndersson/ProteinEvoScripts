...
def superpose_structures(pdb_file1, pdb_file2):
    ...
    coords1 = [ a.coord for a in structure1[0].get_atoms() if a.parent.resname in AA and a.name in atom_types ]
    coords2 = [ a.coord for a in structure2[0].get_atoms() if a.parent.resname in AA and a.name in atom_types ]
...
def calculate_lddt_score(protein1_pdb_path, protein2_pdb_path, inclusion_radius=15.0):
    ...
    for chain1, chain2 in zip(protein1.get_chains(), protein2.get_chains()):
        ...
        for res1, res2 in zip(chain1, chain2):
            ...
            distances = [calculate_distance(ca_coord1, atom2.get_coord()) for atom2 in res2 if atom2.get_name() != "CA"]
            for i, threshold in enumerate(thresholds):
                if any(distance <= (inclusion_radius + threshold) for distance in distances):
                    num_residues_within_thresholds[i] += 1

    fractions_within_thresholds = [num_res / num_residues for num_res in num_residues_within_thresholds]
...
def get_dssp(pdb_file, simplify=False):
    ...
    simplified_string = ''.join(simplified_mapping[symbol] for symbol in secondary_structure)
...
def esm_structs(path):
    ...
    if gpu_name == 0 or 1:
        ...
        available_mems = np.array([get_mem_device(gpu) for gpu in range(torch.cuda.device_count())])
    ...
    for sequence,name in zip(seqs,pname):
        ...
        struct = bsio.load_structure(path[:-5]+name+"ESMfold.pdb", extra_fields=["b_factor"])
        print(name)
        ...
        if dssp_base.count('H') == new_dssp.count('H') and dssp_base.count('E') == new_dssp.count('E'):
            plddt["DSSP"] = "True"
        else:
            plddt["DSSP"] = "False"
...
if not os.path.exists("protein_evo_results.csv"):
    ...
    for fold in fold_types:
        ...
        if os.path.exists(pathdir):
            ...
            print(f"For the fold {fold}, there were {len(file_exist)} files generated. {len(files)-len(file_exist)} files were invalid.")
    ...
    df = pd.DataFrame({"LDDT": lddt_scores, "RMSD": rmsds, "DSSP": dssp_bool, 
                    "Fold": classes, "Accepted": accept_arr, "Paths": filepaths, 
                    "Target DSSP": og_dssp, "New DSSP": calc_dssp})
...
ax1.text(0.5, 2500, f"{sum((df['LDDT'].to_numpy() >= 0.8))} accepted on LDDT", bbox=dict(facecolor='red', alpha=0.5))
ax2.set_xlabel("RMSD")
ax2.text(20, 250, f"{sum((df['RMSD'].to_numpy() <= 5))} accepted on RMSD", bbox=dict(facecolor='blue', alpha=0.5))
ax3.set_xlabel("RMSD")
ax3.text(20, 200, f"{sum((df['DSSP'].to_numpy() == True).astype(int))} accepted on DSSP", bbox=dict(facecolor='green', alpha=0.5))
...
sns.histplot(data=df, x="Fold", hue="Fold", shrink=.8, alpha=.8, legend=False, ax=axs[1])
...
if not os.path.exists('protein_structure_evaluation_dssp.csv'):
    ...
    df_struct = pd.DataFrame(columns=['Path', 'Class', 'Sequence', 'pLDDT', 'DSSP', 'ESM path'])

    for path,i  in zip(dssp_paths, tqdm(range(len(dssp_paths)))):
        temp_dict = esm_structs(path)
        df_struct = df_struct.append(temp_dict, ignore_index=True)
...
sns.displot(data=df_struct, x="pLDDT", col="Class", row="DSSP", facet_kws={'sharey': False})
